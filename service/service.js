...
const convertGuild = (data) => {
  try {
    let guild = (({ profile, member, recentTerritoryWarResult }) => ({
      profile,
      member,
      recentTerritoryWarResult,
    }))(data.guild);
    const allowed = ["playerId", "playerName", "galacticPower"];
    guild.member = guild.member.map((obj) =>
      Object.fromEntries(
        Object.entries(obj).filter(([key, _]) => allowed.includes(key))
      )
    );
    return { guild };
  } catch (error) {
...
const convertPlayer = (data) => {
  try {
    ...
    for (let i = 0; i < player.length; i++) {
      player[i].rosterUnit = player[i].rosterUnit.map((obj) =>
        Object.fromEntries(
          Object.entries(obj).filter(([key, _]) => allowed2.includes(key))
        )
      );
    }
    return player;
...
const filterUnitNames = (text) => {
  try {
    ...
    let filtered = entries.filter(
      (e) =>
        !e.nameKey.startsWith("UNIT_WIN_") &&
        !e.nameKey.startsWith("UNIT_KILL_") &&
        !e.nameKey.startsWith("UNIT_BUNDLE_")
    );
...
function expandSZData(strikeZoneDefinition, tables) {
  const rewardTableMap = new Map(tables.map((table) => [table.id, table.row]));

  return strikeZoneDefinition.map((strikeZone) => {
    const rewardRow = rewardTableMap.get(strikeZone.encounterRewardTableId);
...
function normalizeConflictZones(conflictZones) {
  return conflictZones.map((z) => {
    ...
    return {
      ...
      starThresholds: z.victoryPointRewards.map((r) =>
        Number(r.galacticScoreRequirement)
      ),
...
function buildLocalizationMap(entries) {
  return Object.fromEntries(entries.map((e) => [e.nameKey, e.name]));
}

function applyPlanetNames(planets, localizationMap) {
  for (const planet of planets) {
    planet.name = localizationMap[planet.nameKey] ?? planet.nameKey; // fallback für Debug

    // Optional: StrikeZones ebenfalls benennen
    for (const strike of planet.strikeZones) {
      strike.name = localizationMap[strike.nameKey] ?? strike.nameKey;
    }

    // Optional: ReconZones
    for (const recon of planet.reconZones) {
      recon.name = localizationMap[recon.nameKey] ?? recon.nameKey;
    }
  }
      ...
      for (let s3 = 0; s3 <= 3; s3++) {
        ...
        if (cost <= guildGP) {
          ...
          if (
            ...
            (score === bestStrategy.score && leftover > bestStrategy.leftoverGP)
          ) {
            let plan = [];
 ...
 */
function simulateFullCampaign(
  initialRoster,
  guildTotalGP,
  allPlanetsData,
  strikeZoneSuccessRates = 1.0
) {
  // 1. Raten-Handling: Sicherstellen, dass es ein Array ist
  let ratesArray = Array.isArray(strikeZoneSuccessRates)
    ? strikeZoneSuccessRates
    : Array(6).fill(strikeZoneSuccessRates);
...
function findBestPath(
  ...
  const greedyStrategy = solveRotePhase(
    availableDeploymentGP,
    deploymentPlanetsInput
  );
  ...
  for (const strategy of strategiesToTest) {
    ...
    strategy.plan.forEach((result, index) => {
      ...
      if (result.targetStars >= 1) {
        const unlockedChildren = allPlanetsData.filter(
          (p) =>
            p.unlocksFrom &&
    ...
    let uniqueCandidates = [
      ...new Map(candidatesForNextDay.map((p) => [p.id, p])).values(),
    ];
    ...
    if (combinations.length === 0) {
      if (newTotalStars > globalMaxStars) {
        ...
        globalBestPath = {
          ...
          path: [...currentState.pathHistory, stepLog],
        };
      }
      continue;
    }
...
function processPlanetOperationsDetailed(
  planet,
  rosterPool,
  existingProgressMap,
  dailyGlobalUsedGUIDs,
  ...
  // Fortschritt laden
  const planetProgress = existingProgressMap
    ? existingProgressMap.get(planet.id) || new Map()
    : new Map();
  ...
  allPlatoons.forEach((item) => {
    ...
    if (shouldProcess) {
      ...
      platoonResultsMap.set(item.originalId, {
        ...result,
  ...
  const updatedGlobalProgress = existingProgressMap
    ? new Map(existingProgressMap)
    : new Map();
...
function fillPlatoonRest(
  ...
  const totalSlots = platoon.units.reduce(
    (sum, u) => sum + parseInt(u.amount || 1),
    0
  );
  ...
  for (const req of platoon.units || []) {
    ...
    const eligible = pool.filter(
      (u) => !usedGUIDs.has(u.uuid) && isEligible(u, minRelic)
    );
...
function generatePriorityList(bestPathResult) {
  ...
  bestPathResult.path.forEach((step) => {
    ...
    strategy.plan.forEach((planItem, index) => {
      ...
      const costSoFar =
        planItem.cost +
        planItem.extraDeployment +
        (planItem.opsTP || 0) +
        (planItem.preloadedTP || 0) +
        (planItem.strikeTP || 0);
      ...
      if (
        currentStars < 3 &&
        planItem.thresholds &&
        planItem.thresholds[currentStars]
      ) {
    ...
    if (phaseMap.size > 0) {
      ...
      const sortedUnits = Array.from(phaseMap.values()).sort(
        (a, b) => b.score - a.score
      );
