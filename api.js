...
async function fetchAndSaveGuild() {
  try {
    const response = await axios.post('http://swgoh-comlink:3200/guild', {
      payload: {
...
async function fetchAndSavePlayer() {
  try {
    const requests = player.map((id) =>
      axios.post('http://swgoh-comlink:3200/player', {
        payload: { playerId: id },
        enums: false,
      })
...
async function getLocalizationData() {
  try{
      const response = await axios.post("http://swgoh-comlink:3200/localization", {
          payload: {
...
async function getMetadata() {
  try{
    const response = await axios.post("http://swgoh-comlink:3200/metadata")
...
async function getGameData() {
  try{
      const response = await axios.post('http://swgoh-comlink:3200/data', {
              payload: {
